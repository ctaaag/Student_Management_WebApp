...
function CompareFourChart(props) {
  ...
  const NLNfilter = props.fourWeekNLNData.map((time) => {
    if (time >= parseInt(`${timeLabel[0]}`)) return parseInt(`${timeLabel[0]}`);
      ...
      yaxis: {
        ...
        labels: {
          ...
          formatter: function (val, index) {
            if (index == 6) return `${timeLabel[0]}시간 이상`;
            if (index == 5) return `${timeLabel[1]}시간`;
            if (index == 4) return `${timeLabel[2]}시간`;
            if (index == 3) return `${timeLabel[3]}시간`;
            if (index == 2) return `${timeLabel[4]}시간`;
            if (index == 1) return `${timeLabel[5]}시간`;
            if (index == 0) return `${timeLabel[6]}시간 미만`;
          },
